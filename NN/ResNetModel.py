import torch
import torch.nn as nn

class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(ResidualBlock, self).__init__()
        self.conv1 = nn.Conv1d(in_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv1d(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.skip = nn.Conv1d(in_channels, out_channels, kernel_size=1, stride=1)

    def forward(self, x):
        skip = self.skip(x)
        x = self.conv1(x)
        x = self.relu(x)
        x = self.conv2(x)
        return self.relu(x + skip)

class ResNetModel(nn.Module):
    def __init__(self, num_devices, num_substance_forms, num_measure_types, spectrum_input_size):
        super(ResNetModel, self).__init__()
        

        self.layer1 = ResidualBlock(1, 32)
        self.layer2 = ResidualBlock(32, 64)
        self.global_pool = nn.AdaptiveAvgPool1d(1)
        self.rnfc = nn.Linear(64, 128)
        
        # Metadata Embedding
        self.device_embedding = nn.Embedding(num_devices, 8)
        self.substance_form_embedding = nn.Embedding(num_substance_forms, 8)
        self.measure_type_embedding = nn.Embedding(num_measure_types, 8)
        
        # Metadata Fully Connected
        self.metadata_fc = nn.Sequential(
            nn.Linear(8 * 3, 16),
            nn.ReLU(),
            nn.Dropout(0.2)
        )
        
        # Metadata Projection to Match Spectrum Features
        self.metadata_projection = nn.Linear(16, 128)

        # Fully Connected Layer
        self.fc = nn.Sequential(
            nn.Linear(128 + 128, 128),  # Adjust for concatenated input
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 32),
            nn.ReLU(),
            nn.Linear(32, 1),
        )
    
    def forward(self, inputs):
        spectrum, device_serial, substance_form, measure_type = inputs
        
        # Spectrum through CNN
        spectrum = spectrum.unsqueeze(1)  # Add channel dimension
        
        spectrum_features = self.layer1(spectrum)
        spectrum_features = self.layer2(spectrum_features)
        spectrum_features = self.global_pool(spectrum_features).squeeze(-1)
        spectrum_features = self.rnfc(spectrum_features)

        # Metadata through embeddings
        device_embed = self.device_embedding(device_serial)
        substance_form_embed = self.substance_form_embedding(substance_form)
        measure_type_embed = self.measure_type_embedding(measure_type)
        
        # Combine metadata embeddings
        metadata = torch.cat([device_embed, substance_form_embed, measure_type_embed], dim=1)
        metadata_features = self.metadata_fc(metadata)  # Output size: (B, 16)
        metadata_features = self.metadata_projection(metadata_features)  # Output size: (B, 992)
        
        # Concatenate features
        combined_features = torch.cat([spectrum_features, metadata_features], dim=1)  # Output size: (B, 1984)
        output = self.fc(combined_features)
        return output
