...
class FCCNNModel(nn.Module):
    def __init__(self, num_devices, num_substance_forms, num_measure_types, num_substances, spectrum_input_size):
        ...
        self.substance_form_embedding = nn.Embedding(num_substance_forms, 4)
        self.measure_type_embedding = nn.Embedding(num_measure_types, 4)
        self.predicted_substance_embedding = nn.Embedding(num_substances, 8)
        
        # Metadata Fully Connected
        self.metadata_fc = nn.Sequential(
            nn.Linear(4 + 4 + 4 + 8, 32),
            nn.ReLU(),
            nn.Dropout(0.2)
        )
        
        # Metadata Projection to Match Spectrum Features
        self.metadata_projection = nn.Linear(32, 64)

        # Fully Connected Layer
        self.fc = nn.Sequential(
            nn.Linear(64 + 960, 128),  # Adjust for concatenated input
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 32),
